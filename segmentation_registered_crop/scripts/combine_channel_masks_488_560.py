#!/usr/bin/env python3
"""
Combine 488nm and 560nm per-channel segmentation masks into a union mask.

  <cell_id>_combined_union_488_560.tif  — union (any 1 of 2 channels votes)

Each output TIFF has shape (Z, 5, Y, X) float32:
  ch0  642nm intensity   (from ch488_segmented source — all channels share same registered intensities)
  ch1  488nm intensity
  ch2  560nm intensity
...
Usage:
    python combine_channel_masks_488_560.py [--output-root PATH] [--force] [--dry-run]
...
OUTPUT_ROOT_DEFAULT = Path(
    "/research_jude/rgs01_jude/dept/DNB/core_operations/ImageAnalysis/Core/Haoran/cmap"
...
def compute_2d_boundaries(mask_3d: np.ndarray) -> np.ndarray:
    """
    Compute thick 2D boundary per Z-slice (XY plane).

    Runs find_boundaries independently on each Z-plane so the boundary shows
    the cell outline in every slice.
    """
...
def combine_one_cell(cell_dir: Path, force: bool = False) -> str:
    """
    Combine the 488 and 560 per-channel masks for one cell directory.
    Returns "done", "skipped", "missing: [...]", or "error: <msg>".
    """
    ...
    missing = [t.name for t in [ch488_tif, ch560_tif] if not t.exists()]
    if missing:
        return f"missing: {missing}"

    out_union = cell_dir / f"{cell_id}_combined_union_488_560.tif"

    if not force and out_union.exists():
        return "skipped"

    try:
        arr488 = tifffile.imread(str(ch488_tif))  # (Z, 5, Y, X) float32
        arr560 = tifffile.imread(str(ch560_tif))

        if arr488.shape != arr560.shape:
            return f"error: shape mismatch {arr488.shape} {arr560.shape}"
        if arr488.ndim != 4 or arr488.shape[1] != 5:
            return f"error: unexpected shape {arr488.shape}, expected (Z,5,Y,X)"

        # All per-channel TIFs share the same registered intensity data.
        intensities = arr488[:, :3, :, :].copy()  # (Z, 3, Y, X)

        # Binary masks from channel index 3
        mask488 = (arr488[:, 3, :, :] > 0).astype(np.uint8)
        mask560 = (arr560[:, 3, :, :] > 0).astype(np.uint8)

        union_mask = (mask488 + mask560 >= 1).astype(np.uint8)

        z, _, h, w = arr488.shape
        boundary = compute_2d_boundaries(union_mask)

        out_arr = np.zeros((z, 5, h, w), dtype=np.float32)
        out_arr[:, :3] = intensities
        out_arr[:, 3] = union_mask.astype(np.float32)
        out_arr[:, 4] = boundary.astype(np.float32)

        tmp = str(out_union) + ".tmp"
        tifffile.imwrite(tmp, out_arr, imagej=True, metadata={"axes": "ZCYX"})
        os.replace(tmp, str(out_union))
...
def discover_cell_dirs(output_root: Path, batch: str | None, crop: str | None) -> list[Path]:
    """Find all leaf cell directories (containing *_ch488_segmented.tif files)."""
    cell_dirs = sorted({
        p.parent
        for p in output_root.glob("**/*_ch488_segmented.tif")
    })
    if batch:
