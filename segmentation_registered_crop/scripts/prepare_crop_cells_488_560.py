#!/usr/bin/env python3
"""Stage z±Z_PAD clipped cell crops from 488+560 union masks for canonical feature extraction.

For each cell in ``output_registered_crop_seg/<batch>/<sample>/<cell_id>/``,
loads ``cell_XXXX_combined_union_488_560.tif`` (Z, 5, Y, X), finds the z-range
where the mask channel (index 3) is non-zero, clips to ±Z_PAD slices, and writes
a flat per-sample staging TIF (XY planes only):

    <output_root>/<batch>/<sample>/cell_box_union_488_560/cell_XXXX.tif
...
Usage:
    python prepare_crop_cells_488_560.py [--output-root PATH] [--batch BATCH] [--z-pad N] [--force]
"""
...
Z_PAD_DEFAULT = 5
OUTPUT_SUBDIR = "cell_box_union_488_560"
UNION_SUFFIX = "_combined_union_488_560.tif"
...
def prepare_one_cell(
    ...
    """Clip and stage one cell's combined_union_488_560 TIF.  Returns 'done', 'skip', or 'error:<msg>'."""
    tifs = list(cell_dir.glob(f"*{UNION_SUFFIX}"))
    if not tifs:
        return "error:no combined_union_488_560.tif"
    src = tifs[0]
