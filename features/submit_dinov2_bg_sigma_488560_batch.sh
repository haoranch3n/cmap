#!/bin/bash
# Submit one LSF GPU job per sample for DINOv2 from cell_box_bg_sigma_488560_shape.
# Run AFTER submit_extract_features_bg_sigma_488560_batch.sh features complete.
#
# Usage:
#   bash features/submit_dinov2_bg_sigma_488560_batch.sh [--dry-run]
#
# Override queue/walltime:
#   CMAP_QUEUE=rhel88_gpu CMAP_WALLTIME=4:00 bash features/submit_dinov2_bg_sigma_488560_batch.sh

PROJECT_ROOT="/research_jude/rgs01_jude/dept/DNB/core_operations/ImageAnalysis/Core/Haoran/cmap"
WORKER="${PROJECT_ROOT}/features/_dinov2_bg_sigma_488560_one.sh"
...
LOGDIR="${PROJECT_ROOT}/logs/dinov2_bg_sigma_488560"
mkdir -p "${LOGDIR}"
...
submit_one() {
  ...
  if (( DRY_RUN )); then
    echo "  [dry-run] bsub -J dino_488560_${sample_name} ... ${sample_dir}"
  else
    bsub \
      -n 1 \
      -q "${QUEUE}" \
      -J "dino_488560_${sample_name}" \
      -W "${WALLTIME}" \
      -M "${MEM_MB}" \
...
echo "=== Submitting DINOv2 bg_sigma_488560 batch jobs ==="
...
echo "Logs: ${LOGDIR}"
echo "Monitor: bjobs -J 'dino_488560_*'"
