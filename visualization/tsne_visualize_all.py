#!/usr/bin/env python3
"""
Run t-SNE and UMAP on pooled QC features from all samples.
"""
...
def _collect_features(output_root: Path) -> pd.DataFrame:
    csvs = sorted(output_root.rglob("cell_qc/qc_features.csv"))
    if not csvs:
        raise FileNotFoundError(f"No cell_qc/qc_features.csv found under {output_root}")
    frames = []
    for csv_path in csvs:
        sample_dir = csv_path.parent.parent
    ...
    print(f"\nTotal: {len(combined)} cells from {len(frames)} samples")
    return combined
...
def _rgb_colors(df: pd.DataFrame, col_g: str = "mean_488", col_m: str = "mean_560", clip_pct: float = 99) -> np.ndarray:
    ...
    return np.column_stack([m, g, m, np.full(len(g), 0.7)])
...
def _plot_embedding_grid(df: pd.DataFrame, coords: dict[str, tuple[str, str]], method_name: str, out_path: Path) -> None:
    ...
    n_cells = len(df)
    point_size = max(3, min(40, 2000 // max(n_cells, 1)))
    ...
    for ri, row_label in enumerate(row_labels):
        ...
        for col, title, cmap in panels:
            ax = axes[ri][col_idx]
            vals = df[col].values
            sc = ax.scatter(df[x_col], df[y_col], c=vals, cmap=cmap, s=point_size, alpha=0.7, edgecolors="none", vmin=0, vmax=1000)
        ...
        if has_rgb:
            ax = axes[ri][col_idx]
            ax.scatter(df[x_col], df[y_col], c=rgba, s=point_size, alpha=0.7, edgecolors="none")
    ...
    fig.suptitle(f"All Samples {method_name}  (n={n_cells})", fontsize=16, y=1.01)
    fig.tight_layout()
...
def _compute_embedding(X_scaled, df, perplexity, method, suffix):
    ...
    if method == "tsne":
        ...
        model = TSNE(n_components=2, perplexity=eff_perp, random_state=42, init="pca", learning_rate="auto")
    else:
        try:
            import umap
...
def run(output_root: Path, perplexity: float = 30.0) -> int:
    print(f"Scanning: {output_root}\n")
    df = _collect_features(output_root)
    out_dir = output_root / "cell_qc_all"
    out_dir.mkdir(parents=True, exist_ok=True)
    pooled_csv = out_dir / "all_qc_features.csv"
    df.to_csv(str(pooled_csv), index=False)

    avail_vol = [c for c in FEATURE_COLS if c in df.columns]
    avail_novol = [c for c in FEATURE_COLS_NO_VOL if c in df.columns]
    if not avail_vol:
        print("ERROR: no recognized feature columns")
        return 1
    ...
    for method in ("tsne", "umap"):
        ...
        png_path = out_dir / f"{method}_all.png"
        _plot_embedding_grid(df, coords, label, png_path)

    tsne_csv = out_dir / "tsne_embedding_all.csv"
    df.to_csv(str(tsne_csv), index=False)
    umap_csv = out_dir / "umap_embedding_all.csv"
...
def main() -> int:
    ...
    ap.add_argument("--output-root", type=Path, default=None, help=f"Root output directory to scan (default: {OUTPUT_DIR})")
    ap.add_argument("--perplexity", type=float, default=30.0, help="t-SNE perplexity")
    args = ap.parse_args()
    output_root = args.output_root or Path(OUTPUT_DIR)
    return run(output_root, perplexity=args.perplexity)
